...
def entry_show(
    ...
    console.print(f"[dim]Source   :[/dim] {entry.metadata.source_provenance or '—'}")
    console.print(f"[dim]Status   :[/dim] {entry.metadata.refinement_status.value}")
    console.rule()
...
def extract_file(
    ...
    with console.status(f"Extracting from [bold]{target}[/bold]…"):
        if target.is_dir():
            ...
            extracted = [agent.extract_from_file(target, EntryType(entry_type), tags=tag_list)]
...
def agent_chat(
    ...
    _session = _PS()
    console.print(
        "[bold]Know-Do Graph Agent[/bold]  (type [dim]exit[/dim] or [dim]quit[/dim] to stop, [dim]reset[/dim] to clear history)"
    )
    while True:
        try:
...
def _make_step_handler():
    ...
    def _step_handler(event: str, data: dict) -> None:
        if event == "thinking":
            ...
            console.print(
                f"[dim bold magenta]◈  orchestrator iteration {data['iteration']}…[/dim bold magenta]"
            )
        elif event == "route":
            console.print(
                f"[bold magenta]→  routing to:[/bold magenta] [cyan]{data['agent']}[/cyan]  [dim]{data.get('args', {})}[/dim]"
            )
        elif event == "tool_call":
            args_preview = _json.dumps(data["args"], default=str, ensure_ascii=False)
            if len(args_preview) > 160:
...
def review_run(
    ...
    agent = ReviewAgent(
        app_state.graph, model=model, batch_size=batch_size, on_step=_make_step_handler()
    )
    console.print(
        f"[bold]ReviewAgent[/bold]  batch={batch_size}"
...
orchestrate_app = typer.Typer(
    help="Orchestrator — routes tasks to the right agent", no_args_is_help=True
)
...
embeddings_app = typer.Typer(
    help="Manage vector embeddings for hybrid retrieval", no_args_is_help=True
)
...
def db_merge(
    ...
    api_url: str = typer.Option(
        None,
        "--api-url",
        help="Base URL of the running API (default: $KDG_API_URL)",
    ),
) -> None:
    ...
    console.print(
        f"  entries  inserted={report.entries_inserted}  updated={report.entries_updated}  skipped={report.entries_skipped}"
    )
...
def db_dedup(
    ...
    for c in report.candidates[:20]:
        ...
        console.print(
            f"  {marker}  {c['duplicate_slug']} \u2192 {c['primary_slug']}  ({c['reason']})"
        )
    ...
    if similar > 0:
        ...
        for s in sims[:30]:
            console.print(
                f"  sim={s['similarity']:.3f}  {s['a_id'][:8]}\u2026  \u2194  {s['b_id'][:8]}\u2026"
            )
        if len(sims) > 30:
...
def db_reload(
    api_url: str = typer.Option(
        None, "--api-url", help="Default: $KDG_API_URL or http://127.0.0.1:8000"
    ),
) -> None:
