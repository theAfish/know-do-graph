...
- [ ] Add frontend linting and formatting, for example ESLint plus Prettier or Biome.
- [ ] Add TypeScript or JSDoc typedefs for API payloads, graph nodes, edges, entries, metadata, assets, and UI state.
- [ ] Centralize DOM query helpers and avoid silent failures when required elements are missing.
- [ ] Replace inline `onclick` handlers generated by `panel.js` with delegated event handlers and `data-*` attributes.
- [ ] Add a small API client contract test or mocked frontend test for graph loading, panel rendering, edit submit, delete, search, and SSE refresh.
- [ ] Split CSS by responsibility and document naming conventions. Current files are separated by broad area, but component ownership is still implicit.
- [ ] Add accessible labels, keyboard behavior checks, and focus management tests for the detail panel, toolbar, search, and dialogs.
